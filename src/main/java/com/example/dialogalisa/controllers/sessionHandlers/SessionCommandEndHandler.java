...
public class SessionCommandEndHandler extends YandexAlisRequestAbstractHandler {

    public SessionCommandEndHandler(UserService userService, SessionService sessionService, LessonService lessonService, Session session) {
    ...
    public YandexAliceResponse requestHandler(YandexAliceRequest yandexAliceRequest) {
        ...
        if (command.contains("да") || command.contains("ещё")) {
            ...
            response = createResponse(requestText,requestTts,SessionState.NULLABLE, "");
            return response;
            ...
            if (commandFromSession != null && !commandFromSession.isEmpty()){
                ...
                response = createResponse(requestText,requestTts,SessionState.COMMAND_END, "");
                return response;
            }
            ...
            response = createResponse(requestText,requestTts,SessionState.COMMAND_END, command);
            return response;
        }
    }
